...
class CustomVisitor:
    ...
    def __init__(self, filename):        
        ...
        self.tree = self.readyFile()
        self.root = Node(type(self.tree).__name__)
        self.visit(self.tree, self.root)
    
    def readyFile(self):
        with open(self.filename, 'r', encoding='utf8') as reader:
            code = reader.read()
            tree = ast.parse(code)
        return tree
...
if __name__ == "__main__":        
    visitor = CustomVisitor('target.py')
    local_var = {}
    for ind, statement in enumerate(visitor.root.ast_node):
        print("Local variable value after statement %s execution�F" % (ind+1))
